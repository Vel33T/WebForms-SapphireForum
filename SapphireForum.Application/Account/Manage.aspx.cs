...
namespace SapphireForum.Application.Account
{
    ...
    {
        private const string AVATAR_PATH = "~/Content/Avatars/";
        private const string DEFAULT_AVATAR_NAME = "default-avatar.png";
        ...
        protected void ButtonUpload_Click(object sender, EventArgs e)
        {
            if (FileUploadAvatar.HasFile)
            {
                try
                {
                    if (FileUploadAvatar.PostedFile.ContentType == "image/jpeg")
                    {
                        if (FileUploadAvatar.PostedFile.ContentLength < 102400000)
                        {
                            string userName = User.Identity.GetUserName();
                            FileUploadAvatar.SaveAs(Server.MapPath("~/Content/Avatars/") + userName + ".jpg");
                            ApplicationDbContext context = new ApplicationDbContext();
                            context.Users.FirstOrDefault(u => u.UserName == userName).AvatarLink = userName + ".jpg";
                            context.SaveChanges();
                        }
                    }
                }
